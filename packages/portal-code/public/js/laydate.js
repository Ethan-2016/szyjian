...
; !function (win) {
    ...
    Dates.query = function (node) {
        ...
        var node = (Dates.trim(node)).split(' '), elemId = doc.getElementById(node[0].substr(1)), arr;
        if (!elemId) {
            return;
        } else if (!node[1]) {
            return elemId;
    ...
    Dates.viewtb = (function () {
        ...
        var log = {}, table = doc.createElement('table'), thead = doc.createElement('thead');
    ...
    Dates.view = function (elem, options) {
        ...
        if (!Dates.box) {
            ...
            var html = '<div class="laydate_top">'
    ...
    Dates.events = function () {
        ...
        Dates.each(S('#laydate_table td'), function (i, elem) {
            Dates.on(elem, 'click', function (ev) {
                if (!Dates.hasClass(this, as[1])) {
                    var s = S('#laydate_table td');
                    var leng = s.length;
                    for (var i = 0; i < leng; i++) {
                        if (s[i].className != "undefined") {
                            s[i].className = "";
                        }
                    }
                    this.className = "laydate_click";
                    dater = [this.getAttribute('y') | 0, this.getAttribute('m') | 0, this.getAttribute('d') | 0];
                }

            });
            Dates.on(elem, 'dblclick', function (ev) {
                if (!Dates.hasClass(this, as[1])) {
                    Dates.stopmp(ev);
