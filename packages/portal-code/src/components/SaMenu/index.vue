...
export default {
  ...
  methods: {
    clickItem(index) {
      console.log(index);
      if (index == 0) this.$router.push({ path: "/saSchedule/saSchedule" });
      if (index == 1) this.$router.push({ path: "/saManagement/saOverview" });
      if (index == 2)
        this.$router.push({ path: "/saDictionary/saTermDictionary" });
      if (index == 3)
        this.$router.push({ path: "/saSystem/saDocumentConfiguration" });
    },
  },
};
</script>
